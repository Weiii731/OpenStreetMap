...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    //     std::cout << "Node x: " << node->x << " Node y: " << node->y << " Sum:" << node->g_value + node->h_value << "\n";
    // }
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // vector should start with start_node and end with end_node;
    path_found.emplace_back(*current_node);
    RouteModel::Node* cur = current_node;
    RouteModel::Node* parentNode = current_node->parent;
    while (parentNode != nullptr)
    {
        path_found.emplace_back(*parentNode);
        distance += parentNode->distance(*cur);
        cur = parentNode;
